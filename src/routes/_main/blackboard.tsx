...
function BlackboardPage() {
  ...
  const notes = [
    {
      id: 1,
      content: "I still remember the scent of the cherry blossoms near the North Library every April. It felt like time stood still, and for a moment, the stress of finals just vanished into the pink petals.",
    ...
    {
      id: 2,
      content: "Professor Miller's 8 AM History lectures were the reason I chose my career path. He taught us that the past is never truly gone; it's a living, breathing thing that shapes every decision we make today.",
    ...
    {
      id: 3,
      content: "Late night ramen runs. The foundation of our friendships.",
    ...
    {
      id: 4,
      content: "Met my wife at the Fall mixer in '94. Thirty years later, we are coming back for the reunion with our grandkids in tow. It's beautiful how life comes full circle on this campus.",
      author: "David Chen",
    ...
    {
      id: 5,
      content: "The old oak tree by the quad. I wonder if students still hide notes in its bark like we used to.",
      author: "Jameson Lee",
    ...
    {
      id: 6,
      content: "Winning the championship in 2004. That final shot still plays in my head every single night.",
      author: "Kira Vance",
    ...
    {
      id: 7,
      content: "Freshman year, I got lost trying to find the lecture hall and ended up in a senior seminar. The professor let me stay, and it was the best accidental lesson I ever had.",
      author: "Leo Zhang",
      department: "Sciences",
      year: "2019",
      color: "bg-[#e0f2f1]", 
  ...
  const filteredNotes = useMemo(() => {
    return notes.filter((note) => {
      const matchesSearch = note.author.toLowerCase().includes(searchTerm.toLowerCase()) ||
  ...
  const getRotation = (id: number) => {
    const rotations = ['rotate-1', '-rotate-1', 'rotate-2', '-rotate-2', 'rotate-[0.5deg]', '-rotate-[1.5deg]']
  ...
  <div className="text-center max-w-2xl mx-auto space-y-4">
    ...
    <p className="text-lg text-stone-500 font-medium leading-relaxed">
      Every graduate leaves a story behind — a lesson learned, a friendship formed,
      a struggle overcome, or advice worth passing on.
    </p>
        ...
        >
            ...
            {paginatedNotes.map((note) => (
              ...
              >
                ...
                <div className="absolute top-1 left-1/2 -translate-x-1/2 z-30 flex flex-col items-center">
                  <div className={`w-5 h-5 rounded-full ${note.pinColor} shadow-[0_2px_4px_rgba(0,0,0,0.3)] border border-white/20`} />
                  <div className="w-0.5 h-2 bg-stone-600/40 -mt-1 blur-[0.5px]" />
                </div>

                {/* Sticky Note */}
                <Card className={`${note.color} border-none rounded-sm shadow-[6px_6px_12px_rgba(0,0,0,0.15)] flex flex-col h-full overflow-hidden`}>
                  <CardContent className="p-8 flex flex-col h-full min-h-55">
                    ...
                    <div className="flex items-center gap-3 pt-4 border-t border-black/5">
                      ...
                      <div className="min-w-0 gap-1">
                        <p className="text-[13px] font-bold text-stone-900 truncate tracking-tight">{note.author}</p>
          ...
          {filteredNotes.length === 0 && (
            <div className="text-center py-32">
              <p className="text-white/60 font-serif text-2xl italic tracking-wide">No memories pinned here yet...</p>
            </div>
          )}
        </div>
          ...
          <div className="mt-12 flex items-center justify-center gap-8">
            ...
            <div className="flex items-center gap-4">
              <span className="text-[11px] font-bold uppercase tracking-[0.3em] text-stone-400">
                Memories <span className="text-stone-900">{currentPage}</span> / {totalPages}
              </span>
            </div>
