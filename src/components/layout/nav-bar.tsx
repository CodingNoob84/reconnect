...
export const Navbar = () => {
  ...
  const navItems = [
    ...
    ...(isAuthenticated ? [{ title: 'My Profile', icon: BookOpen, href: '/myprofile' }] : []),
  ]
        ...
        <div className="flex items-center justify-start min-w-30">
          <Link to="/" className="group flex items-center gap-2">
            <h1 className="font-serif text-2xl font-black tracking-tight text-foreground">
              Re<span className="italic text-primary transition-colors duration-300 group-hover:text-primary/80">Connect</span>
            </h1>
          </Link>
        </div>

        {/* Desktop Navigation - Added md:flex-nowrap and whitespace-nowrap to keep it in one line */}
          ...
          {isAuthenticated ? (
            <div className="flex items-center gap-4">
              ...
              <div className="hidden flex-col items-end text-right md:flex whitespace-nowrap">
                ...
                <span className="text-[10px] font-bold uppercase tracking-widest text-primary/80">Alumni</span>
              </div>
              ...
              <Avatar className="h-10 w-10 ring-2 ring-primary/10 transition-all duration-300 hover:scale-105 hover:ring-primary/40 cursor-pointer shadow-sm">
                ...
                <AvatarFallback className="bg-primary/5 text-xs font-bold text-primary">AR</AvatarFallback>
              </Avatar>
          ...
          <div className="flex items-center md:hidden">
            <Sheet>
              <SheetTrigger asChild>
                <Button variant="ghost" size="icon" className="h-10 w-10 rounded-xl bg-muted/50 hover:bg-muted border border-border/50">
                  <Menu className="h-5 w-5 text-foreground" />
                </Button>
              </SheetTrigger>
              <SheetContent side="right" className="flex w-75 flex-col justify-between bg-background/95 p-6 backdrop-blur-md">
                    ...
                    {navItems.map((item) => {
                      ...
                      return (
                        <Link
                          ...
                          to={item.href}
                          activeProps={{
                            className: "bg-primary text-primary-foreground shadow-md shadow-primary/10 font-semibold",
                          }}
                          inactiveProps={{
                  ...
                  <div className="border-t border-border/60 pt-4 space-y-4">
                    <div className="flex items-center gap-3 px-2">
                      ...
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] font-bold uppercase tracking-wider text-primary/80">Alumni</span>
                      </div>
                    </div>
                    <Button 
                      variant="ghost" 
